...
- **本地访问**：`http://localhost:8501`（默认，仅本机可访问）

**如需公网访问**：在 `docker-compose.yml` 中将 `ports` 改为：
```yaml
- "8501:8501"
```
然后重启服务，并确保防火墙开放 8501 端口。

确保防火墙已开放 8501 端口（公网访问时），例如：
...
# 重新构建并启动
./deploy.sh stop
./deploy.sh install   # 或 docker compose build
./deploy.sh start
```

数据保存在 `/www/maigen/`，更新不会丢失。
...
| `/www/maigen/videos` | 下载的谱面视频（`videos/downloads`） |
| `/www/maigen/b50_datas` | 成绩图片、生成的视频（按用户名分目录） |
| `/www/maigen/static_user` | 自定义视频模板（背景图、字体、音乐等，对应 `static/user`） |
| `/www/maigen/user_config` | 用户偏好 |
| `/www/maigen/config` | 全局配置（`global_config.yaml`） |

详见 [生成文件路径说明](FILE_PATHS.md)。
...
在 Docker 或 SSH 无图形界面环境下，可采用以下方式之一：

1. **Web 内扫码登录（推荐）**  
   进入左侧 **B站登录** 页面，在网页中直接显示二维码，用 B站 App 扫描即可。页面会每 2 秒自动检查登录状态。
