...
from utils.PageUtils import download_temp_image_to_static, read_global_config
...
def custom_po_token_verifier() -> Tuple[str, str]:
    ...
    if config.get('CUSTOMER_PO_TOKEN', {}).get('visitor_data') == "" or config.get('CUSTOMER_PO_TOKEN', {}).get('po_token') == "":
...
class BilibiliDownloader(Downloader):
    ...
    def log_in(self, credential_path):
        # 无头/服务器环境（无图形界面）时使用终端二维码，否则使用 tkinter 弹窗
        import os
        if os.environ.get("DISPLAY") or os.name == "nt":
            credential = login.login_with_qrcode()  # 使用 tkinter GUI 显示二维码
        else:
            credential = login.login_with_qrcode_term()  # 在终端打印二维码，适用于 Docker/SSH
        try:
