...
# 支持 Docker 部署：优先使用 /app/config/global_config.yaml（持久化卷）
def _get_global_config_path():
    """获取全局配置文件路径，Docker 部署时使用持久化卷"""
    docker_config = os.path.join(os.path.dirname(__file__), "..", "config", "global_config.yaml")
    local_config = "global_config.yaml"
...
def read_global_config():
    ...
    if os.path.exists(config_path):
        ...
        raise FileNotFoundError(f"global_config.yaml not found (checked: {config_path})")
...
def get_db_manager() -> "DatabaseManager":
    ...
    if 'db_manager' not in st.session_state:
        ...
        db_path = os.environ.get("MAI_DB_PATH", "mai_gen_videob50.db")
...
def clear_all_user_data(username: str) -> dict:
    ...
    try:
        ...
        # 3. 清空配置文件中的敏感信息
        config_file = _get_global_config_path()
        if os.path.exists(config_file):
            try:
                with open(config_file, 'r', encoding='utf-8') as f:
